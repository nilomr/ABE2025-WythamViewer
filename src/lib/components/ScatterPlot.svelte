<script lang="ts">
  ...
  // Select items arrays
  $: speciesSelectItems = [{ value: 'All', label: 'All' }, ...speciesOptions.map(s => ({ value: s, label: s }))];
  $: siteSelectItems = [{ value: 'All', label: 'All' }, ...siteOptions.map(s => ({ value: s, label: s }))];
  ...
  async function fetchData() {
    ...
    data = d3.csvParse(csvData, (d: any, i: number) => {
      ...
      const confidence = d.confidence !== undefined && d.confidence !== '' ? parseFloat(d.confidence) : null;
  ...
  function updatePointColors() {
    ...
    const speciesPalette = speciesList.map((_, i) => interpolateSpectral(i / Math.max(1, speciesList.length - 1)));
    const sitePalette = siteList.map((_, i) => interpolateSpectral(i / Math.max(1, siteList.length - 1)));
    ...
    data.forEach((point, i) => {
      ...
      if (isActive) {
        if (colorMode === 'time') {
          ...
          const paletteIdx = paletteLen > 1 && uniqueTimes.length > 1
  ...
  function getCameraDistanceToFitSphere(radius: number, camera: THREE.PerspectiveCamera, fitRatio = 1.2): number {
